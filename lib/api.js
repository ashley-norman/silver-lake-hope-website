...
export async function getAllCalendarEvents(preview) {
  const entries = await fetchGraphQL(
    `query {
      calendarEventsCollection(preview: ${preview ? "true" : "false"}) {
        items {
          sys {
            id
          }
  ...
  return entries?.data?.calendarEventsCollection?.items
}
